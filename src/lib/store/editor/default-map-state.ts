 ...
 */
export const defaultMapState = derived<typeof mapSize, GridMap>(mapSize, ({ gridSize }) => {
    const defaultMap = Array.from({ length: gridSize[0] }, () => Array(gridSize[1]).fill(Tileset.EMPTY))
    return defaultMap;
})