 ...
 */
function parseBooleanEnv(value: string | undefined, defaultValue = false, varName = 'UNKNOWN'): boolean {
    ...
    if (truthyValues.includes(normalized)) {
        console.log(`[ENV Parse] ${varName}: Parsed "${value}" -> true (truthy)`);
        return true;
    }

    if (falsyValues.includes(normalized)) {
        console.log(`[ENV Parse] ${varName}: Parsed "${value}" -> false (falsy)`);
        return false;
    }

    // Use Boolean() for other cases
    const result = Boolean(normalized);
    console.log(`[ENV Parse] ${varName}: Parsed "${value}" -> ${result} (Boolean cast)`);
    return result;
}
...
function parseNumberEnv(
    ...
    const { allowFloat = false, min = Number.NEGATIVE_INFINITY, max = Number.POSITIVE_INFINITY } = options;

    console.log(`[ENV Parse] ${varName}:`, {
        raw: value,
    ...
    if (Number.isNaN(parsed)) {
        console.warn(`[ENV Parse] ${varName}: Invalid number "${value}", using default`, defaultValue);
        return defaultValue;
    }

    if (parsed < min || parsed > max) {
        console.warn(`[ENV Parse] ${varName}: Value ${parsed} out of range [${min}, ${max}], using default`, defaultValue);
        return defaultValue;
    }

    console.log(`[ENV Parse] ${varName}: Parsed "${value}" -> ${parsed} (${allowFloat ? 'float' : 'int'})`);
    return parsed;
}
...
function parseStringEnv(
    ...
    const { required = false, minLength = 0, maxLength = Number.POSITIVE_INFINITY } = options;
    ...
    if (!value || value === '') {
        if (required) {
            console.error(`[ENV Parse] ${varName}: Required environment variable is missing`);
    ...
    if (trimmed.length < minLength || trimmed.length > maxLength) {
        console.warn(`[ENV Parse] ${varName}: String length ${trimmed.length} out of range [${minLength}, ${maxLength}], using default`, defaultValue);
        return defaultValue;
    }

    console.log(`[ENV Parse] ${varName}: Parsed "${value}" -> "${trimmed}"`);
    return trimmed;
}
...
function loadAnimationConfig(): AnimationConfig {
    try {
        ...
        console.log('[AnimationConfig] Environment variables loaded successfully:', config);
        return config;
    } catch (error) {
        console.error('[AnimationConfig] Failed to load environment variables:', error);
